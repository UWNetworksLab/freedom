/// <reference path="typescript-defs/MediaStream.d.ts" />
/// <reference path="typescript-defs/RTCPeerConnection.d.ts" />
...
export enum SimpleDataPeerState {
  DISCONNECTED = 0,
  CONNECTING = 1,
  CONNECTED = 2
};

export class SimpleDataPeer {
  private constraints : MediaConstraints;

  constructor (public peerName: string, public pcConfig?: string[]) {
    pcConfig = static_pc_config;
    this.constraints = {optional: [{DtlsSrtpKeyAgreement: true}]};
  }
...
// };
function DataPeer(peerName, dataChannelCallbacks, stunConfig) {
  this.peerName = peerName;
  this._simplePeer = new SimpleDataPeer(this.peerName, stunConfig);
